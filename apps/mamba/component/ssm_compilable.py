...
def _ssm_chunk_scan_combined_fwd_fake(
    ...
    return (
        ...
        x.new_empty((cu_seqlens.size(0) - 1, n_heads, head_dim, B.size(0)))
...
if __name__ == "__main__":
    ...
    out = compiled_mamba_chunk_scan_combined(
        x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias
    )

    print(out.min(), out.max(), out.mean(), out.std())

    out_ref = mamba_chunk_scan_combined_ref(
        x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias
    )
